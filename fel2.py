...
def parse_symbol(string, start):
    ...
    number = True
    while string[end] not in " \t\n'()":    # om inte är en tab, ny linje eller ' samt () 
        if string[end] not in "0123456789.": # om det inte är numreriska värden
            number = False
        end += 1
    ...
    if result == '':
        number = False    
    if number:
        return float(result), end    #ValueError: could not convert string to float: vilket borde vara om det är text
    else:
        return result, end
...
def parse_list(string, start):
    ...
    while string[start] != ')':
        ...
        if part != '':                                                      # om inte är tom så skall vi lägga till i result[] 
            result.append(part)
